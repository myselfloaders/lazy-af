...
@Client.on_message(filters.command(['torrent@Leech_VLbot', 'ziptorrent@Leech_VLbot', 'filetorrent@Leech_VLbot']) & filters.chat(ALL_CHATS))
...
async def initiate_torrent(client, message, link, flags):
    ...
    reply = await message.reply_text('Accepted')
    ...
    try:
        ...
        await asyncio.gather(message.reply_text(f'Give Me With Good Seeders\n{ex.error_code}: {html.escape(ex.error_message)}'), reply.delete())
...
@Client.on_message(filters.command(['magnet@Leech_VLbot', 'zipmagnet@Leech_VLbot', 'filemagnet@Leech_VLbot']) & filters.chat(ALL_CHATS))
...
async def initiate_magnet(client, message, link, flags):
    ...
    try:
        ...
        await asyncio.gather(message.reply_text('The Seeders Sucks'), reply.delete())

    else:

        await handle_leech(client, message, gid, reply, user_id, flags)

@Client.on_message(filters.command(['directdl', 'stream', 'stream@Leech_VLbot', 'zipdirectdl@Leech_VLbot', 'zipdirect@Leech_VLbot', 'filedirectdl@Leech_VLbot', 'document@Leech_VLbot', 'document']) & filters.chat(ALL_CHATS))
...
async def initiate_directdl(client, message, link, filename, flags):
    ...
    try:
        ...
        await asyncio.gather(message.reply_text(f'Invalid Link Provided or believe the error must be this.{ex.error_code}: {html.escape(ex.error_message)}'), reply.delete())

    except asyncio.TimeoutError:

        await asyncio.gather(message.reply_text('Invalid link provided or unsupported'), reply.delete())
...
<b>╰─⌊ETA⌉</b> {calculate_eta(completed_length, total_length, start_time)}'''
    ...
    if torrent_info['status'] == 'error':
        ...
        if upload_queue._unfinished_tasks:
            ...
            task = asyncio.create_task(reply.edit_text('Uploading Please Wait...'))
        ...
        try:
            ...
            if not (ex.error_code == 1 and ex.error_message == f'I cannot find anything with this GID#{gid}'):
...
@Client.on_message(filters.command(['list', 'list@Leech_VLbot']) & filters.chat(ALL_CHATS))
...
@Client.on_message(filters.command(['cancel', 'cancel@Leech_VLbot']) & filters.chat(ALL_CHATS))
