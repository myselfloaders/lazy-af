...
message_info = dict()
ignore = set()
@Client.on_message(filters.command(['anime', 'anime@Leech_VLbot']))
...
@Client.on_message(filters.command(['hentai', 'hentai@Leech_VLbot']))
...
async def init_search(client, message, query, sukebei):
    ...
    if not result:
        await message.reply_text('Try something that I understand')
    else:
...
'''/anime <b>Search Anime</b>

/hentai <b>Search Hentai</b>''')
